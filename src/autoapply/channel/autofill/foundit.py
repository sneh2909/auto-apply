"""Foundit (formerly Monster India) easy-apply.
...
class FounditAutoFiller(ATSAutoFiller):
    name = "foundit"
    ...
    async def apply(self, application_id: str) -> None:
        ...
        try:
            result = await apply_on_portal(
                portal="foundit",
                job_url=a.ats_url,
                profile_dir=settings.foundit_browser_dir,
                qa_bank=qa_bank,
            ...
            a.error = None
            await a.save()
            log.info("foundit apply %s -> %s", application_id, result)
